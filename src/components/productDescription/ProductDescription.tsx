...
export function ProductDescription({ product }: Props) {
  ...
  const quantity = cart?.products.find((item) => item.id === product.id)?.quantity || 0
    ...
    <div className={styles['description-block-container']}>
      <div className={styles['description-title']}>
        ...
        <div className={styles['other']}>
          <span className={styles['other-text']}>{product.warrantyInformation}</span>
          <span className={styles['other-text']}>{product.shippingInformation}</span>
        </div>
